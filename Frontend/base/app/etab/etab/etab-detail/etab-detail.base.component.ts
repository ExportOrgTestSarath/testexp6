...
import { EtabBase} from '../etab.base.model';
...
import { EtabApiConstants } from '@baseapp/etab/etab/etab.api-constants';
...
export class EtabDetailBaseComponent{
...
confirmationReference:any;
  ...
  "actions" : {
	...
	pageViewTitle: string = 'ETAB_DETAIL';
	
	public etabService = inject(EtabService);
...
step:workflowInfo.step
       ...
       const methodName: any = "configureFormOnWorkflow";
        let action: Exclude<keyof EtabDetailBaseComponent, ' '> = methodName;
...
showMessage(config:any){
  ...
  if (environment.prototype && this.id) {
    ...
    this.etabService.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const dataSubscription = this.etabService.getById (params).subscribe((res: EtabBase[]) => {
      this.data = res || {};
      this.formatRawData();
    });
...
workflowInformation.step = workflowDataForNewRec.step || '';
	...
	editFormula(config:any,fieldname:any){
     ...
     if(fieldConfig.formulaMethod){
       let methodName: Exclude<keyof EtabDetailBaseComponent, ' '> = fieldConfig.formulaMethod
  ...
  const requestedObj = this.generateRequestObj(data);
      ...
      const saveSubscription = this.uploadAttachmentsandSaveData(requestedObj, splittedData,actionName).subscribe(res => {
        ...
        const saveSubscription = this.etabService[method](requestedObj,actionName).subscribe((res:EtabBase) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any,actionName?:string): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.etabService.create(splittedData.data,actionName).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.etabService.update(dataToUpdate.data,actionName).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
