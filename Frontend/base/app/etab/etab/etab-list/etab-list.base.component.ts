...
export class EtabListBaseComponent{
...
localStorageStateKey = "etab-list";
...
@ViewChild('toggleButton')
 ...
 filtersApplied:boolean = false;
  ...
  "rowHeight" : "medium"
	...
	pageViewTitle: string = 'ETAB_LIST';
...
public location = inject(Location);
 ...
 if(!(this.appUtilBaseService.isEqualIgnoreCase(value,this.filter.quickFilter, [],true))){
    ...
    return {
      ...
      ajaxUrl: EtabApiConstants.getDatatableData,
      select: true,
...
getParentId() {
  ...
  let property: Exclude<keyof EtabListBaseComponent, ' '> = value;
  if (this.isChildPage) {
    if (this[property]) {
...
focus(){
  ...
  this.tableSearchFieldConfig= this.appUtilBaseService.getControlsFromFormConfig(this.tableSearchConfig)
    ...
    let action: Exclude<keyof EtabListBaseComponent, ' '> = methodName;
...
onExportSucess() {
  ...
  this.showAdvancedSearch = !this.showAdvancedSearch;
    ...
    if (environment.prototype && this.tableConfig.children?.length > 0) {
      gridSubscription = this.etabService.getProtoTypingData().subscribe((data: any) => {
    ...
    if (this.selectedValues.length > 0) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          const deleteSubscription = this.etabService.delete(requestedParams).subscribe((res: any) => {
...
this.location.back();
	...
	if (this.isChildPage && this[property]) {
		...
		this.router.navigate(['../etabdetail'], { relativeTo: this.activatedRoute});
	}
}
	clearGlobalSearch(){
  this.filter.globalSearch = '';
