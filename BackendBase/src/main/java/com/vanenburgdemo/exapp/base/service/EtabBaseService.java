...
public class EtabBaseService<T extends EtabBase> extends BaseBusinessLogic<T>
		implements IEtabBaseService<T> {
		
	private static final Logger LOGGER = LoggerFactory.getLogger(EtabBaseService.class.getName());
	...
	@Autowired
	private  EtabPerimeterBaseImpl etabPerimeterBaseImpl;
	
	
	public EtabBaseService(Class<T> modelClass) {
		super(modelClass);
		...
		registerModelTransformer(PersistenceType.ANALYTICAL, new EtabAnalyticalTransformer());
	}
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("Etab", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("Etab", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("Etab", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
